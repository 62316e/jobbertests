...
async function main() {
    ...
    try {
        ...
        if (noBuild) {
            console.error(`Job bundle not found: ${path.relative(process.cwd(), bundlePath)}\nRun: tsx scripts/build-jobs.ts`);
            process.exit(1);
        }
        console.log('Bundle not found, building jobs...');
        const { spawn } = await import('node:child_process');
        await new Promise((resolve, reject) => {
            const p = spawn(process.platform === 'win32' ? 'npx.cmd' : 'npx', ['tsx', 'scripts/build-jobs.ts'], {
                stdio: 'inherit',
                cwd: path.join(__dirname, '..')
            });
            p.on('exit', (code) => (code === 0 ? resolve() : reject(new Error(`Build failed with code ${code}`))));
        });
    }
